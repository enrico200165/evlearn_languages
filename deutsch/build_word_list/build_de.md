...
Fonti consigliate: 
Leipzig Corpora Collection, perché offre corpora gratuiti e statistiche corpus-based in formato scaricabile;

Leipzig:  
[https://wortschatz.uni-leipzig.de/en](https://wortschatz.uni-leipzig.de/en) /   
[https://aclanthology.org/L12-1154/](https://aclanthology.org/L12-1154/); 

## Script 1 - Tedesco da Leipzig Corpora Collection
...
Oppure, se si vuole scaricare direttamente un archivio Leipzig:

```
python build_core_de_from_leipzig.py ^
    --url "URL_ARCHIVIO_LEIPZIG_TAR_GZ" ^
    --output-dir output_de ^
    --max-rank 5000
```

Nota: Leipzig distribuisce corpora in archivi che normalmente contengono file come `*_sentences.txt`.   Lo script cerca automaticamente un file `sentences.txt` dentro l’archivio e costruisce una lista lemmatizzata.  
Le liste Leipzig/Wiktionary basate su frequenza sono spesso liste di forme ortografiche, non lemmi; per un software didattico conviene ricondurre le forme al lemma.  
Wiktionary segnala esplicitamente che molte frequency list contano parole ortografiche distinte, non lemmi: [https://en.wiktionary.org/wiki/Wiktionary:Frequency_lists](https://en.wiktionary.org/wiki/Wiktionary:Frequency_lists). ([Wiktionary][2])

File: `build_core_de_from_leipzig.py`
...
Gli script producono file senza overlap:

```
core_1000_de.csv      rank 1-1000
core_2000_de.csv      rank 1001-2000
core_3000_de.csv      rank 2001-3000
core_4000_de.csv      rank 3001-4000
core_5000_de.csv      rank 4001-5000
```
...
Per il tedesco, Leipzig è ottimo come corpus gratuito e comparabile, ma la qualità didattica finale dipende molto dalla lemmatizzazione spaCy e dal tipo di corpus scelto, ad esempio news, web o Wikipedia. 
