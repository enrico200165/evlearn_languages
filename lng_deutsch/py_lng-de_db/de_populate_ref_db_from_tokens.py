...
# File token usato quando il programma viene lanciato senza argomento posizionale.
# È intenzionalmente relativo alla directory corrente, così può essere cambiato
# facilmente da chi lancia lo script oppure sostituito passando un argomento CLI.
DEFAULT_INPUT_TOKEN_FILE = Path("lng_deutsch/build_word_list/input_lists/input_tokens.txt")
DEFAULT_LOG_FILE_NAME = "de_populate_ref_db_from_tokens.log"
...
def setup_logger(
    *,
    ...
    """Create a logger with concise console output and detailed file output.

    Every log record includes timestamp, source Python file and source line
    number. The function clears previous handlers because this module can be
    used both as a CLI and as an imported component during tests or larger
    orchestrations.
    """
    ...
    logger = logging.getLogger("de_populate_ref_db_from_tokens")
    ...
    file_handler = logging.FileHandler(
        log_dir_path / DEFAULT_LOG_FILE_NAME,
        encoding="utf-8",
    )
    ...
    # Reuse the same handlers for the linguistic module.
    # In questo modo il log dettagliato spiega anche COME viene cercata
    # l'informazione: cache UniMorph, download HTTP, Wiktionary REST, spaCy locale,
    # template deterministici, fallback identità.
    for child_logger_name in ("de_token2lexinfo",):
    ...
    logger.debug("Logger inizializzato. log_file=%s", log_dir_path / DEFAULT_LOG_FILE_NAME)
...
def parse_token_file(
    ...
    *,
    ...
    if logger is not None:
        logger.info("Lettura file token: %s", path)
    headers: dict[str, str] = {}
    token_lines: list[TokenLine] = []
        ...
        for line_number, raw in enumerate(handle, start=1):
            ...
            if parsed_header is not None:
                ...
                if key == "marker" and value:
                    marker = value
                    if logger is not None:
                        logger.info("Marker metadati rilevato: %s", marker)
                if logger is not None:
                    logger.debug("Header letto: %s=%s", key, value)
                continue
    ...
    if logger is not None:
        logger.info("Token letti: %s", len(token_lines))
        logger.debug("Header complessivi: %s", headers)
...
def populate_de_reference_db_from_token_file(
    ...
    *,
    ...
    if logger is None:
        effective_log_dir = Path(log_dir).expanduser().resolve() if log_dir is not None else db_dir_path / "_logs"
        logger = setup_logger(log_dir=effective_log_dir)

    logger.info("Avvio popolamento reference DB tedesco")
    logger.info("Input token file: %s", token_file)
    logger.info("Database path: %s", db_path)

    parsed = parse_token_file(token_file, logger=logger)

    conn = open_de_ref_db(db_dir=db_dir_path, db_file_name=db_file_name)
    ...
    try:
        total_tokens = len(parsed.token_lines)
        logger.info("Token da elaborare: %s", total_tokens)

        for index, token_line in enumerate(parsed.token_lines, start=1):
            token = token_line.token
            logger.debug(
                "Elaborazione token index=%s/%s line=%s token=%r",
                index,
                total_tokens,
            ...
            if log_every > 0 and (index == 1 or index % log_every == 0 or index == total_tokens):
                logger.info(
                    "Elaborazione token index=%s/%s line=%s token=%s",
                    index,
                    total_tokens,
                    token_line.line_number,
                    token,
            ...
            try:
                ...
                logger.debug(
                    "Lemma prodotto: token=%r lemma=%r source=%s",
                    token,
                    lemma,
                    lemma_result.source,
                )
                ...
                if lemma_id is None:
                    logger.info("Nuovo lemma da inserire: %s", lemma)
                    ...
                    if lemma_id is None:
                        ...
                        inserted_lemmas += 1
                        logger.debug("Lemma inserito: id=%s lemma=%s", lemma_id, lemma)
                    else:
                        existing_lemmas += 1
                        logger.info(
                            "Lemma già presente dopo analisi completa: id=%s lemma=%s; non inserisco un duplicato",
                            lemma_id,
                            lemma,
                        )
                else:
                    existing_lemmas += 1
                    logger.info(
                        "Lemma già presente: id=%s lemma=%s; salto build_de_lexical_reference e registro solo l'occorrenza",
                        lemma_id,
                        lemma,
                    )
                ...
                input_occurrences += 1
                logger.debug(
                    "Occorrenza input registrata: lemma_id=%s token=%r line=%s",
                    lemma_id,
                    token,
                    token_line.line_number,
                )
                ...
                if commit_every > 0 and index % commit_every == 0:
                    conn.commit()
                    logger.info("Commit intermedio dopo %s token", index)

            except Exception as exc:
                skipped_lines += 1
                logger.exception(
                    "Riga saltata: line=%s token=%r errore=%s",
                    token_line.line_number,
                    token,
                    exc,
                )
        ...
        conn.commit()
        logger.info(
            "Popolamento completato: total=%s inserted=%s existing=%s occurrences=%s skipped=%s",
            len(parsed.token_lines),
            inserted_lemmas,
            existing_lemmas,
            input_occurrences,
            skipped_lines,
        ...
        logger.debug("Chiusura connessione database")
        conn.close()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "token_file",
        nargs="?",
        default=str(DEFAULT_INPUT_TOKEN_FILE),
        help=(
            "UTF-8 token list file. One German token per line. "
            f"Default: {DEFAULT_INPUT_TOKEN_FILE}"
        ),
    )
    ...
    parser.add_argument(
        "--log-dir",
        default=None,
        help="Directory for detailed log file. Default: <db-dir>/_logs."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Log progress to console every N tokens. Default: 50."
    )
